import { haversineDistance, gaussianWeight, windToComponents, barnesInterpolation, toRadians } from './src/barnesInterpolation';
...
it('should correctly convert wind speed and direction to U and V components compared to manually calculated values', () => {
  ...
  it('should calculate the correct distances between marker pairs', () => {
    ...
    const distance = haversineDistance(marker1.coords[0], marker1.coords[1], marker2.coords[0], marker2.coords[1]);
  ...
  it('should interpolate wind vectors at specific target points', () => {
    ...
    const { U, V } = barnesInterpolation(formattedData, targetLat, targetLon, kappa, 1);
  ...
  it('should interpolate wind vectors with multiple passes for refinement', () => {
    ...
    const { U, V } = barnesInterpolation(formattedData, targetLat, targetLon, kappa, passes);
